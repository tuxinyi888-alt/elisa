...
LOESS-style connecting lines through the group means.
"""
...
def read_plate_od(csv_path):
    ...
    with open(csv_path) as fh:
        lines = fh.readlines()
    # Find the 405 nm data lines (format: ,A,val1,val2,...,405)
    for line in lines:
        ...
        if row_letter in "ABCDEFGH" and wavelength == "405":
            for col_idx in range(2, 14):  # columns 1-12
                col_num = col_idx - 1
                try:
...
def parse_layout_block(layout_df, start_row):
    ...
    Returns:
      std_types: {row_letter: type} e.g. {'A': 'Blk', ...}
      std_concs: {(row_letter, col): concentration} for cols 2,3
      sample_ids: {(row_letter, col): sample_name} for cols 4-12
    """
    std_types = {}
    std_concs = {}
    sample_ids = {}
    for i in range(8):
        ...
        # Columns 2-3 are standard concentrations
        for c in [2, 3]:
            try:
...
def fit_std_curve_and_predict(od_data, std_types, std_concs, sample_ids):
    ...
    1. Compute NSB (mean of NSB control wells col 1)
    2. Compute B0 (mean of B0 control wells col 1)
    3. %B/B0 for standards and samples
    4. Fit 4PL to standards, predict sample concentrations
    """
    # NSB and B0 from column 1 control wells
    ...
    # Standards: columns 2-3
    std_points = []  # (conc, pct_b_b0)
    ...
    # Fit 4PL  (competitive: slope is typically negative in logistic form)
    try:
        popt, _ = curve_fit(
    ...
    # Invert: given %B/B0, find concentration
    ...
    for (row, col), sid in sample_ids.items():
        plate_col = col  # layout column index
        # Map layout columns 4-12 to plate columns 4-12
...
def parse_sample_id(sid):
    """
    Parse 'Exp1 C0 L10 (1/1)' → (donor, carprofen, stimulus, stim_conc).
    """
...
def process_pge2():
    ...
    # Identify plate block start rows in layout
    # Plate 2 block starts at layout row index 21 (row 20 is the header)
    # Plate 3 block starts at layout row index 31
    plate_configs = [
    ...
    for cfg in plate_configs:
        ...
        # Map layout columns (4–12) to plate reader columns (4–12)
        # Layout column indices are 4-12; plate reader columns are 4-12
        # But layout is 0-indexed from CSV, and plate cols are 1-12
        # Layout col 4 → plate col 4, layout col 5 → plate col 5, ...
...
# ═══════════════════════════════════════════════════════════════════════════
# 3. Combine all cytokines, filter to LPS only
# ═══════════════════════════════════════════════════════════════════════════
all_data = pd.concat([pge2, tnfa, il1b, il6], ignore_index=True)
lps = all_data[all_data["Stimulus"] == "LPS"].copy()
lps = lps.dropna(subset=["value"])
...
LPS_DOSES = [0, 1, 10, 100]
# Map real LPS dose to evenly-spaced x position
LPS_POS = {0: 0, 1: 1, 10: 2, 100: 3}
LPS_LABELS = ["0", "1", "10", "100"]
...
def make_plot_single_panel():
    ...
    Each panel has LPS dose on x, cytokine on y, stratified by carprofen.
    """
    for cyto in CYTOKINE_ORDER:
        subset = lps[lps["cytokine"] == cyto]
        if subset.empty:
            print(f"  ⚠ No data for {cyto} (LPS), skipping.")
            continue
        ...
        for cdose in CARPROFEN_DOSES:
            ...
            x_pos = grp["Stim_Conc_num"].map(LPS_POS).values + JITTER[cdose]
            ax.scatter(
                x_pos, grp["value"],
            ...
            summary = summary.reindex(LPS_DOSES)
            valid = summary.dropna(subset=["mean"])
            if valid.empty:
                continue

            x_mean = np.array([LPS_POS[d] for d in valid.index]) + JITTER[cdose]
            ax.errorbar(
        ...
        ax.set_xticks(list(LPS_POS.values()))
        ax.set_xticklabels(LPS_LABELS)
        ax.set_xlabel("LPS dose", fontsize=13, fontweight="bold")
        ...
        fig.tight_layout()
        fname = f"LPS_dose_response_{cyto.replace('-', '').replace('α', 'a').replace('β', 'b')}_by_carprofen.png"
...
def make_plot_grid():
    ...
    for idx, cyto in enumerate(CYTOKINE_ORDER):
        ...
        ax.set_xticks(list(LPS_POS.values()))
        ax.set_xticklabels(LPS_LABELS)
        ax.set_xlabel("LPS dose", fontsize=11)
    ...
    fig.suptitle(
        "Cytokine dose-response to LPS, stratified by carprofen dose",
        fontsize=15, fontweight="bold", y=1.01,
    )
    fig.tight_layout(rect=[0, 0.03, 1, 0.99])
    fname = "LPS_dose_response_all_cytokines_by_carprofen.png"
...
def make_plot_facet_grid():
        ...
        for col_idx, cdose in enumerate(CARPROFEN_DOSES):
            ...
            if not grp.empty:
                ...
                for donor in grp["Donor"].unique():
                    ...
                    x_pos = ddata["Stim_Conc_num"].map(LPS_POS).values
                    ax.scatter(
                        x_pos, ddata["value"],
                ...
                valid = summary.dropna(subset=["mean"])
                if not valid.empty:
                    x_mean = np.array([LPS_POS[d] for d in valid.index])
                    ax.errorbar(
                        x_mean, valid["mean"], yerr=valid["sem"],
            ...
            ax.set_xticks(list(LPS_POS.values()))
            ax.set_xticklabels(LPS_LABELS, fontsize=9)
            ax.spines["top"].set_visible(False)
            ax.spines["right"].set_visible(False)
    ...
    fname = "LPS_dose_response_facet_grid.png"
    fig.savefig(os.path.join(BASE, fname), dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved {fname}")
...
print("\nData summary (LPS only, non-NA):")
for cyto in CYTOKINE_ORDER:
    n = lps[lps["cytokine"] == cyto].shape[0]
